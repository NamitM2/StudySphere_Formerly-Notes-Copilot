...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-black">
      {/* Header */}
      <header className="border-b border-emerald-950/50 bg-black/90 backdrop-blur-sm sticky top-0 z-50">
        <div className="mx-auto max-w-7xl px-6 py-5">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-gradient-to-br from-emerald-600 to-emerald-800 rounded-lg flex items-center justify-center shadow-lg shadow-emerald-600/20">
                <span className="text-black font-bold text-lg">N</span>
              </div>
              <div className="text-2xl font-bold bg-gradient-to-r from-emerald-500 to-emerald-400 bg-clip-text text-transparent">
                Notes Copilot
              </div>
            </div>
            ...
            <div
              className={`ml-4 flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-medium transition-all duration-300 ${
                isSignedIn
                  ? "bg-emerald-950/60 text-emerald-500 border border-emerald-900/60"
                  : "bg-red-950/50 text-red-400 border border-red-800/50"
              }`}
            >
              <div className={`w-2 h-2 rounded-full ${isSignedIn ? "bg-emerald-500 animate-pulse" : "bg-red-400"}`} />
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-8">
          ...
          <aside className="space-y-6">
            {/* Auth Card */}
            <section className="bg-zinc-950 border border-emerald-950/40 rounded-2xl p-6 shadow-2xl shadow-emerald-600/5 hover:shadow-emerald-600/10 transition-shadow duration-500">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-emerald-400">Authentication</h3>
                {isSignedIn && (
                  <button
                    onClick={doSignOut}
              ...
              {!isSignedIn ? (
                <>
                    ...
                    {["signin", "signup"].map((m) => (
                      <button
                        ...
                        className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 ${
                          mode === m
                            ? "bg-emerald-700 text-black shadow-lg shadow-emerald-600/25"
                  ...
                  <input
                    className="w-full px-4 py-3 mb-3 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-100 placeholder-zinc-500 focus:border-emerald-700/50 focus:ring-2 focus:ring-emerald-700/20 outline-none transition-all duration-200"
                    placeholder="you@example.com"
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                  />
                  <input
                    className="w-full px-4 py-3 mb-4 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-100 placeholder-zinc-500 focus:border-emerald-700/50 focus:ring-2 focus:ring-emerald-700/20 outline-none transition-all duration-200"
                  ...
                  <button
                    className="w-full py-3 rounded-lg font-medium transition-all duration-300 bg-gradient-to-r from-emerald-700 to-emerald-600 text-black hover:from-emerald-600 hover:to-emerald-500 disabled:opacity-50 disabled:cursor-default shadow-lg shadow-emerald-600/25 hover:shadow-emerald-600/40 hover:scale-[1.02] active:scale-[0.98]"
                ...
                <div className="space-y-2">
                  <div className="text-emerald-500 font-medium">{userEmail}</div>
            ...
            {/* Library Card */}
            <section className="bg-zinc-950 border border-emerald-950/40 rounded-2xl p-6 shadow-2xl shadow-emerald-600/5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-semibold text-emerald-400">Document Library</h3>
                <button
                  className="text-xs text-emerald-500 hover:text-emerald-400 transition-colors duration-200 disabled:opacity-50"
                  onClick={refreshDocs}
              ...
              {!isSignedIn ? (
                ...
                <ul className="space-y-2 max-h-96 overflow-y-auto">
                  {docs.map((d) => (
                    <li
                      key={d.doc_id || d.id}
                      className="group rounded-lg border border-zinc-800 bg-zinc-900/50 p-3 hover:bg-zinc-900 hover:border-emerald-900/40 transition-all duration-200"
                    >
          ...
          <div className="space-y-8">
            {/* Hero */}
            <section className="relative rounded-3xl bg-gradient-to-br from-emerald-950/60 via-zinc-950 to-emerald-950/40 p-8 pt-10 pb-10 border border-emerald-950/40 shadow-2xl hover:shadow-emerald-600/20 transition-all duration-500 hover:border-emerald-900/60 group">
              <div className="absolute inset-0 bg-gradient-to-br from-emerald-700/10 via-transparent to-emerald-600/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-3xl" />
              <div className="relative">
                <h1 className="text-4xl font-bold bg-gradient-to-r from-emerald-500 via-emerald-400 to-green-500 bg-clip-text text-transparent leading-relaxed pb-1">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              {/* Upload Card */}
              <section className="bg-zinc-950 border border-emerald-950/40 rounded-2xl p-6 shadow-2xl shadow-emerald-600/5 h-fit">
                <h2 className="text-2xl font-bold text-emerald-400 mb-2">Upload Documents</h2>
                ...
                <div
                  ...
                  className={`relative border-2 border-dashed rounded-xl p-4 text-center transition-all duration-300 ${
                    isDragging ? "border-emerald-600 bg-emerald-950/30" : "border-zinc-800 bg-zinc-900/30 hover:border-emerald-900/60 hover:bg-zinc-900/50"
                  ...
                  {file ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <button
                        ...
                        className="px-5 py-2 rounded-lg bg-emerald-700 text-black font-medium hover:bg-emerald-600 transition-all duration-300 shadow-lg shadow-emerald-600/25 hover:scale-[1.05] active:scale-[0.95]"
                        disabled={!isSignedIn}
                      >
                        Browse Files
                      </button>
                ...
                <button
                  ...
                  className="mt-3 w-full py-2.5 rounded-lg font-medium bg-gradient-to-r from-emerald-700 to-emerald-600 text-black hover:from-emerald-600 hover:to-emerald-500 disabled:opacity-50 disabled:cursor-default transition-all duration-300 shadow-lg shadow-emerald-600/25 hover:scale-[1.02] active:scale-[0.98]"
                >
              ...
              {/* Q&A Card */}
              <section className="bg-zinc-950 border border-emerald-950/40 rounded-2xl p-6 shadow-2xl shadow-emerald-600/5 h-fit">
                <h2 className="text-2xl font-bold text-emerald-400 mb-2">Ask Questions</h2>
                <p className="text-sm text-zinc-500 mb-4">Get AI-powered insights from your documents</p>

                <textarea
                  className="w-full h-24 px-4 py-3 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-100 placeholder-zinc-500 focus:border-emerald-700/50 focus:ring-2 focus:ring-emerald-700/20 outline-none transition-all duration-200 resize-none text-sm"
                ...
                <button
                  ...
                  className="mt-3 w-full py-2.5 rounded-lg font-medium bg-gradient-to-r from-emerald-700 to-emerald-600 text-black hover:from-emerald-600 hover:to-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 shadow-lg shadow-emerald-600/25 hover:scale-[1.02] active:scale-[0.98]"
                >
                  {asking ? (
            ...
            {/* Answer section */}
            {answer && (
              <div className="rounded-2xl border border-emerald-900/40 bg-emerald-950/30 p-6 shadow-2xl shadow-emerald-600/10">
                <div className="mb-4">
                  {hasSourceCitations ? (
                    <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-emerald-900/40 text-emerald-400 border border-emerald-700/50">
                      From Notes
                    </span>
                  ) : (
                    <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-purple-900/40 text-purple-400 border border-purple-700/50">
                      Model Knowledge
                    </span>
                  )}
                </div>
